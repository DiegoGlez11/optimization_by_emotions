...
def apply_mixup(dataset_file,window_size,label,yes_or_not): # initial empty label arrays
    ...
    '''
    print( data_inter_cnn.shape ,data_inter_rnn.shape, label_inter.shape, record)
    #(2400, 128, 9, 9) (2400, 128, 32) (2400,) s05.dat
    
    #return shuffled_data_cnn ,shuffled_data_rnn,shuffled_label,record
    return data_inter_cnn ,data_inter_rnn, label_inter, record

if __name__ == '__main__' :
    begin = time.time()
    print("time begin:",time.localtime())
    dataset_dir		=   "../../data_input/data_preprocessed_python/"
    window_size		=	128
    #output_dir		=   "./deap_shuffled_data/"       # 清洗（打乱）数据
    output_dir = "./deap_shuffled_data/"
    label_class = "dominance"  #sys.argv[1]  # arousal/valence

...
# pos de los metadatos según la posición en el array global
METADATA_ARRAY = []
...
# carga una poblacion
def load_objectives_space( id_pareto, is_non_dominated):
    global all_fronts, METADATA_REG, METADATA_ARRAY, num_individuals_by_front
    
    # if is_non_dominated:
    #     res_obj = load_object_space_ros_non_dom(id_pareto, num_individuals_by_front, return_object=True)
    # else:
    #     res_obj = load_object_space_ros(id_pareto, return_object=True)

    res_obj = load_object_space_ros_non_dom(id_pareto, num_individuals_by_front, return_object=True, non_dominated=is_non_dominated)
    
    data = res_obj["obj_space"]

    # se guarda el frente
    if all_fronts is None:
        all_fronts = data
        pos_range = {"ini": 0, "end": all_fronts.shape[0]}
    else:
        ini_r = all_fronts.shape[0]
        # se guarda la info de los ind
        all_fronts = np.vstack((all_fronts, data))
        pos_range = {"ini": ini_r, "end": all_fronts.shape[0]}

    # nuevo registro de id
    metadata = {"range": pos_range, "id_pareto_front": id_pareto}
    metadata["num_inds"] = np.array(res_obj["num_inds"])
    
    # se guarda los datos del frente por su ID
    METADATA_REG[id_pareto] = metadata

    # se guardan por posición en array
    r = pos_range["end"] - pos_range["ini"]
    METADATA_ARRAY = METADATA_ARRAY + [metadata] * r

    return metadata

# carga una lista de frentes por su ID. Si en un elemento de la lista 
# no hay una lista en lugar de un ID entonces se combinan los frentes de esa lista 
# para formar un nuevo frente
def load_fronts(front_table, ini_pos):
    global METADATA_REG, METADATA_ARRAY, all_fronts

    # reinicio de los valores frentes
    all_fronts = None
    METADATA_ARRAY = []
    METADATA_REG = {}

    # carga de los frentes
    for i in range(len(front_table)):
        id_pareto = front_table[i]
        
        # carga del frente
        if i == 0:
            metadata = load_objectives_space( id_pareto, is_non_dominated=False)
        else:
            metadata = load_objectives_space( id_pareto, is_non_dominated=True)
        
        metadata["pos_id_pareto_front"] = i + ini_pos

    print("inds cargados",all_fronts.shape)



# control preference
def control_pref(req):
    global emotion_values, id_experiments, num_inds
    ...
    if req.reset_table and req.user_name != "":
        ...
        load_table(req.user_name, start=True)
        emotion_values = np.empty((0, 2))
        num_inds = []
    ...
    if req.id_pareto_front != "" and req.user_name != "":
        ...
        load_table(req.user_name, id_pareto=req.id_pareto_front)
...
def load_table(user_name, ini=None, start=False, id_pareto=None):
...
def predictions(msg):
    ...
    if emotion_values.shape[0] >= num_individuals_by_front:
        ...
        point_pref, num_ind_pref = preference_emo_model(emo_v, num_i, id_pareto)
    ...
    print(METADATA_REG.keys())


def preference_emo_model(emotion_values, num_inds, id_pareto):
    ...
    # id_exp = id_experiments[pos_max]
    # num ind
    num_ind = num_inds[pos_max]
    
    # extraemos sus metadatos
    metadata = METADATA_REG[id_pareto]

    # punto de referencia
    pos_ref = metadata["range"]["ini"] + num_ind

    point_ref = all_fronts[pos_ref]
...
def next_front(user_name, point_reference, id_pareto, num_ind_ref, storage_pos):
    ...
    # metadatos del frente
    meta_ref = METADATA_REG[id_pareto]
    # valores del punto de referencia
    point_reference = all_fronts[meta_ref["range"]["ini"] : meta_ref["range"]["end"]]
    point_reference = point_reference[num_ind_ref]
    print("point_reference", point_reference)

    # pos del frente actual en la tabla
    actual_front = get_pos_table(user_name)

    # frentes que superan el umbral de búsqueda
    pos_ini_search = actual_front + search_threshold_front

    # carga del rango actual
    if(pos_ini_search >= RANGE_LOADED["end"]):
        print("load_sub table: pos_ini_search >= RANGE_LOADED['end']")
        # nueva subtabla
        id_front_search = load_table(user_name, ini=pos_ini_search)
    else:
        print("SUBTABLE")

        p = pos_ini_search - RANGE_LOADED["ini"]
        id_front_search = SUBTABLE[p]
        print("pos_ini_search < RANGE_LOADED['end'] pos relative",p, " id_front",id_front_search)
        

    # metadatos del primer frente en la nueva búsqueda
    metadata = METADATA_REG[id_front_search]

    # inicio del rango de búsqueda
    ini = metadata["range"]["ini"]

    # se extraen las referencias de los individuos de los frentes 
    # pertenecientes a la búsqueda
    search_fronts = all_fronts[ini:]

    # chebyshev
    chebyshev_dist = np.abs(search_fronts - point_reference)
    chebyshev_dist = np.max(chebyshev_dist, axis=1)

    # se busca el individuo con menor distancia al ref point
    pos_close = np.argpartition(chebyshev_dist, 1)
    # posición
    pos_close = pos_close[0] + ini
    # valor del individuo
    point_close = all_fronts[pos_close]

    # metadatos de point_close
    metadata_close = METADATA_ARRAY[pos_close]
    # id del frente al que pertenece point_close
    id_pareto_close = metadata_close["id_pareto_front"]

    # pos en la tabla de frentes
    actual_front = metadata_close["pos_id_pareto_front"]
    # se actualiza la posición actual de la tabla
    update_pos_table(user_name, pos=actual_front)
    print(f"from:{id_front_search}, new_front:{id_pareto_close}, metadata_close:{metadata_close}")
    # print("METADATA_REG", [(METADATA_REG[f]["id_pareto_front"], METADATA_REG[f]["range"]) for f in METADATA_REG])

    # rango de almacenamiento de los individuos del frente al que pertenece point_close
    ini_close = metadata_close["range"]["ini"]
    end_close = metadata_close["range"]["end"]

    # vector director de point_close al resto de individuos del frente
    search_vector = all_fronts[ini_close : end_close] - point_close   
    ...
    search_vector = np.argpartition(search_vector, num_individuals_by_front)
    
    # individuos seleccionados
    individuals_selected = search_vector[ : num_individuals_by_front]
    individuals_selected = metadata_close["num_inds"][individuals_selected]  


    pos_close = pos_close - ini_close
    num_ind_close = int(metadata_close["num_inds"][pos_close])
    print(f"pos_close {pos_close} num_ind_close {num_ind_close}")

    save_data(user_name, storage_pos, id_pareto, point_reference, num_ind_ref, id_pareto_close, point_close, num_ind_close, individuals_selected)

    print_color(f"New front:{id_pareto_close}  old:{id_pareto}  indsel:{individuals_selected}",bcolors.OKCYAN)
    print("-----------------------------------------------------")
...
def save_data(user_name, storage_pos, id_pareto, point_reference, num_ind_ref, id_pareto_close, point_close, num_ind_close, individuals_selected):
    ...
    num_ind_ref_point = int(num_ind_ref)
    point_close = point_close.tolist()
    individuals_selected = individuals_selected.tolist()
    

    # se almacenan los individuos
    mref = { "id_pareto_front": id_pareto, "num_ind": num_ind_ref_point, "point": point_reference }
    ...
    # señal de último frente de la tabla
    last_front = get_last_front()
    if last_front == id_pareto_close:
        try:
            srv = rospy.ServiceProxy("/experiment_control", experiment_control)
            srv("last_front", "",[],False,False)
    ...
    # msg_next.num_ind_ref_point = num_ind_ref_point
...
def get_dist_inds(req):
    ...
    points = load_object_space_ros_non_dom(id_pareto)
